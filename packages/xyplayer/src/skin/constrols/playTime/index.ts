 ...
 * @Descripttion:
 * @LastEditTime: 2021-09-24 11:48:46
...
const skin_playTime = function (this: Player) {
  ...
  const onTimeupdate = () => {
    playTimeEl.textContent = this.config.isLive ? `${format(this.video.currentTime)}` : `${format(this.video.currentTime)} / ${format(this.duration)}`;
  }
